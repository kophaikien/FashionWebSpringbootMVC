...
<body data-pc-preset="preset-1" data-pc-direction="ltr" data-pc-theme="light">
  ...
  <div class="pc-container">
    <div class="pc-content">
          ...
          <div class="row align-items-center">
            <div class="col-md-12">
              <div class="page-header-title">
                <h5 class="m-b-10">Thống kê lợi nhuận</h5>
              </div>
              <ul class="breadcrumb">
                <li class="breadcrumb-item"><a href="${path}/admin/home">Home</a></li>
                <li class="breadcrumb-item active" aria-current="page">Thống kê lợi nhuận</li>
              </ul>
            </div>
          </div>
        </div>
      ...
      <form action="${path}/admin/profit/view" method="get">
              ...
              <tbody>
                <c:forEach var="order" items="${filteredOrders}">
                  <tr>
                    ...
                    <td><fmt:formatDate value="${order.createDate}" pattern="dd/MM/yyyy HH:mm:ss" /></td>
                    <td><fmt:formatDate value="${order.updateDate}" pattern="dd/MM/yyyy HH:mm:ss" /></td>
                    <td>${order.payment}</td>
                    <td>${order.status}</td>
                    <td><fmt:formatNumber value="${order.totalAmount}" type="currency" currencySymbol="đ" /></td>
                    <td><fmt:formatNumber value="${order.totalImportCost}" type="currency" currencySymbol="đ" /></td>
                    <td><fmt:formatNumber value="${order.totalProfit}" type="currency" currencySymbol="đ" /></td>
                    <td><a href="${path}/admin/saleOrder/saleOrder-edit/${order.id}">Xem chi tiết</a></td>
                  </tr>
                </c:forEach>
              </tbody>
            </table>
